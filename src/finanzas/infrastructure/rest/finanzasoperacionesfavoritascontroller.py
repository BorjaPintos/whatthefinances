...
def list_operaciones_favoritas(params: dict) -> Tuple[Any, int]:
    ...
    elements, total_elements = list_operaciones_favoritas_use_case.execute(params)
    response_elements = []
...
def create_operacion_favorita(params: dict) -> Tuple[Any, int]:
    ...
    if created:
        ...
        code = 400
        logger.warning("Error al crear la operacion favorita: {}".format(params.get("nombre")))
        raise MessageError("Error al crear la operacion favorita: {}".format(params.get("nombre")), code)
