...
function load_chart() {
    $.get("finanzas/monedero/" + id_monedero, function (monedero) {
        var cantidad_inicial = monedero.cantidad_inicial || 0;

        $.get("finanzas/movimiento_monedero?id_monedero=" + id_monedero + "&order_property=fecha&order_type=asc&count=10000&offset=0", function (response) {
...
function create_chart_evolucion(labels, data) {
    var ctx = document.getElementById('evolucion-monedero');
    if (chart) {
        chart.destroy();
    }
    chart = new Chart(ctx, {
...
$(document).ready(function () {
    id_monedero = $('#id_monedero').val();

    if (!id_monedero || id_monedero === 'None') {
        return;
    }
    ...
    table = $('#lista_tabla')
        ...
        .on('xhr.dt', function (e, settings, json, xhr) {
            ...
            $.ajax({
                url: "finanzas/monedero/" + id_monedero,
                async: false,
                dataType: 'json',
                success: function (monedero) {
                    cantidad_inicial = monedero.cantidad_inicial || 0;
                }
            });

            $.ajax({
                url: "finanzas/movimiento_monedero?id_monedero=" + id_monedero + "&order_property=fecha&order_type=asc&count=10000&offset=0",
                async: false,
                dataType: 'json',
        ...
        .DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '/finanzas/movimiento_monedero',
                data: function (d) {
                    d.id_monedero = id_monedero;
